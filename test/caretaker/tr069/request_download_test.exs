defmodule Caretaker.TR069.RequestDownloadTest do
  ...
  test "RequestDownload encode/decode and response" do
    ...
    {:ok, %{body: %{rpc: "RequestDownload", xml: xml}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env))

    assert {:ok, %RequestDownload{file_type: "1 Firmware", file_type_arg: "vendor:arg"}} =
             RequestDownload.decode(xml)

    {:ok, body2} = RequestDownloadResponse.encode(%RequestDownloadResponse{status: 1})
    {:ok, env2} = SOAP.encode_envelope(body2, %{id: "RD2"})

    {:ok, %{body: %{rpc: "RequestDownloadResponse", xml: xml2}}} =
