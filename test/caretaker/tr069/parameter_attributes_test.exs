defmodule Caretaker.TR069.ParameterAttributesTest do
  ...
  test "GetParameterAttributes encode/decode" do
    ...
    {:ok, %{body: %{rpc: "GetParameterAttributes", xml: xml}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env))

    assert {:ok, %GetParameterAttributes{names: ["Device.", "Device.ManagementServer."]}} =
             GetParameterAttributes.decode(xml)
  end

  test "GetParameterAttributesResponse encode/decode" do
    resp = %{
      parameters: [%{name: "Device.", notification: 0, access_list: ["Subscriber", "ACS"]}]
    }

    {:ok, body} = GetParameterAttributesResponse.encode(resp)
    {:ok, env} = SOAP.encode_envelope(body, %{id: "PA2"})

    {:ok, %{body: %{rpc: "GetParameterAttributesResponse", xml: xml}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env))
  ...
  test "SetParameterAttributes encode/decode and response" do
    ...
    {:ok, %{body: %{rpc: "SetParameterAttributes", xml: xml}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env))
    ...
    {:ok, %{body: %{rpc: "SetParameterAttributesResponse", xml: xml2}}} =
