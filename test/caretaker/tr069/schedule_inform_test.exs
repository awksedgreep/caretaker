defmodule Caretaker.TR069.ScheduleInformTest do
  ...
  test "ScheduleInform encode/decode and response" do
    ...
    {:ok, %{body: %{rpc: "ScheduleInform", xml: xml}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env))

    assert {:ok, %ScheduleInform{delay_seconds: 120, command_key: "CKS"}} =
             ScheduleInform.decode(xml)

    {:ok, body2} = ScheduleInformResponse.encode(%ScheduleInformResponse{})
    {:ok, env2} = SOAP.encode_envelope(body2, %{id: "SI2"})

    {:ok, %{body: %{rpc: "ScheduleInformResponse", xml: xml2}}} =
