defmodule Caretaker.TR069.AutonomousTransferCompleteTest do
  ...
  test "AutonomousTransferComplete encode/decode and response" do
    ...
    {:ok, %{body: %{rpc: "AutonomousTransferComplete", xml: xml}}} =
    ...
    {:ok, body2} =
      AutonomousTransferCompleteResponse.encode(%AutonomousTransferCompleteResponse{})

    {:ok, env2} = SOAP.encode_envelope(body2, %{id: "AT2"})

    {:ok, %{body: %{rpc: "AutonomousTransferCompleteResponse", xml: xml2}}} =
      SOAP.decode_envelope(IO.iodata_to_binary(env2))

    assert {:ok, %AutonomousTransferCompleteResponse{}} =
             AutonomousTransferCompleteResponse.decode(xml2)
  end
end
