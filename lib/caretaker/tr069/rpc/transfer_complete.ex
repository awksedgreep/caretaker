defmodule Caretaker.TR069.RPC.TransferComplete do
  @moduledoc """
  TR-069 TransferComplete (CPE -> ACS).
  """

  @enforce_keys [:command_key, :start_time, :complete_time, :fault_code, :fault_string]
  defstruct [
    :command_key,
    :start_time,
    :complete_time,
  ...
  def decode(xml) when is_binary(xml) do
    try do
      ...
      with {:ok, parsed} <- Lather.Xml.Parser.parse(wrapped) do
        root = parsed["root"] || %{}
        node = root["cwmp:TransferComplete"] || root["TransferComplete"] || %{}
        f = node["FaultStruct"] || %{}
