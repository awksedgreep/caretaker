defmodule Caretaker.TR069.RPC.ScheduleDownloadResponse do
  @moduledoc """
  TR-069 ScheduleDownloadResponse (CPE -> ACS).
  """
  ...
  def decode(xml) when is_binary(xml) do
    try do
      ...
      with {:ok, parsed} <- Lather.Xml.Parser.parse(wrapped) do
        root = parsed["root"] || %{}
        node = root["cwmp:ScheduleDownloadResponse"] || root["ScheduleDownloadResponse"] || %{}
