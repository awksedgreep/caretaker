defmodule Caretaker.TR069.RPC.RequestDownload do
  ...
  def new(opts),
    do: %__MODULE__{
      ...
      file_type_arg: Keyword.fetch!(opts, :file_type_arg)
    }
  ...
  def encode(%__MODULE__{} = r) do
    map = %{
      "cwmp:RequestDownload" => %{"FileType" => r.file_type, "FileTypeArg" => r.file_type_arg}
    }
  ...
  def decode(xml) when is_binary(xml) do
    try do
      ...
      with {:ok, parsed} <- Lather.Xml.Parser.parse(wrapped) do
        ...
        {:ok,
         %__MODULE__{file_type: node["FileType"] || "", file_type_arg: node["FileTypeArg"] || ""}}
      end
    rescue
