defmodule Caretaker.TR069.RPC.DownloadResponse do
  @moduledoc """
  TR-069 DownloadResponse (CPE -> ACS).
  """
  ...
  @doc "Encode body element via Lather"
  ...
  @doc "Decode body element via Lather"
  ...
  def decode(xml) when is_binary(xml) do
    try do
      ...
      with {:ok, parsed} <- Lather.Xml.Parser.parse(wrapped) do
        root = parsed["root"] || %{}
        node = root["cwmp:DownloadResponse"] || root["DownloadResponse"] || %{}

        {:ok,
         %__MODULE__{
           status: to_int(node["Status"], 0),
